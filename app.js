var topics = ["חתול", "כלב", "שבלול", "בית", "סוס", "ענן", "עץ", "מכונית", "פיצה", "טלפון", "הליקופטר", "אננס", "תפוח", "טלויזיה", "דיסק ישן", "חייזר", "שרק", "ספר", "טוסטר", "נעל", "כיסא", "דינוסאור", "לטאה", "מחשב", "עכבר", "מחצלת", "אש", "שיניים", "כובע", "טיל", "רכבת", "בקבוק", "גיטרה", "גרב", "דלעת", "אגס", "בננה", "סירה", "צמח", "שעון", "מנורה", "כובע ישן"];//I will add more words soon.
var timeouts = [];
...
newColor();
var code, ref, topic = "<div class='error'>שגיעה</div>", players = {}, playercode, scoresLeft, ratedPlayers = [];
function score(n){
...
function rateDrawing(){
	document.getElementById("prompt").innerHTML = "דרג ציור זה<div id='rating'>טוב ממש <div class='star' onclick='score(1)'></div><div class='star' onclick='score(2)'></div><div class='star' onclick='score(3)'></div><div class='star' onclick='score(4)'></div><div class='star' onclick='score(5)'></div> נוראי</div>";
	document.getElementById("p").setAttribute("d", "");
...
function newGame(){
	...
	database.ref("/A" + code).once("value", function(e){
		if(e.val() == null){
			...
			document.getElementById("container").innerHTML = "<div id='code' class='font'>" + code + "</div><div id='players' class='font'>שתף קוד זה בכדי להכניס עוד שחקנים למשחק</div><div id='start' class='font' onclick='startGame()'>!התחל</div>";
			...
			database.ref("/A" + code + "/status").on("value", function(e){
				...
				if(e == 1){
					...
					document.getElementById("container").innerHTML = "<div id='loader' class='font'>התכוננו לצציר בעוד <span id='count'>5</span></div>";
					...
					timeouts.push(window.setTimeout(function(){
						...
						function countDown(){
							...
							if(n != 0)
								...
								document.getElementById("prompt").innerHTML = "<div class='topic'>!נגמר הזמן</div>";
								timeouts.push(setTimeout(function(){
				...
				if(e == 2){
					...
					p.setAttribute("transform", "");
					document.getElementById("svg").style.transform = "unset";
					document.getElementById("prompt").innerHTML = "<div class='topic'>:הכי טוב במשחק</div>";
					// if(!mobile)
					requestAnimationFrame(positionDrawing);
					timeouts.push(setTimeout(function(){
...
function joinGame(e){
	e.innerHTML = "<input id='input' class='font' placeholder='קוד המשחק' onkeyup='checkCode(this)' autofocus></input>";
	e.className += " notturn";
	e.onclick = null;
}
function checkCode(el){
	if(el.value.length == 6){
		...
		database.ref("/A" + code + "/status").once("value", function(e){
			if(e.val() === 0){
				document.getElementById("container").innerHTML = "<div id='waiting' style='background-color: " + getColor(color) + "'><span class='wait font'>..מחכה להתחלת המשחק</span></div><div id='wcode' class='wait font'>" + code + "</div>";
				ref = database.ref("/A" + code).push();
				ref.set({
					color: color,
					d: "",
				...
				database.ref("/A" + code + "/status").on("value", function(e){
					...
					if(e == 1){
						...
						ratedPlayers = [];
						d = "M" + document.body.clientWidth / 2 + "," + document.body.clientHeight / 2 + " ";
						document.getElementById("container").innerHTML = "<div id='loader' class='font'>...התכוננו לצייר<span id='count'>5</span></div>";
						timeouts.push(window.setTimeout(function(){
							document.getElementById("count").innerHTML = "4";
						}, 1000));
						timeouts.push(window.setTimeout(function(){
